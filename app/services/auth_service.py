...
from app.utils.users_utils import is_valid_password


class AuthService:
    @classmethod
    async def create_token(cls, user_id: UUID, uow: UOW) -> Token:
    ...
    @classmethod
    async def authenticate_user(cls, email: str, password: str, uow: UOW) -> ModelType:
        async with uow:
            db_user, err = await uow.users.find_one(email=email)
            if err:
                raise HTTPException(
